...
public class AbbvieTest extends CoParticipacaoTest {
	...
	private static final int NUM_TOTAL_TITULARES_FATUCOPA = 466;
	private static final int NUM_TOTAL_DEPENDENTES_FATUCOPA = 565;
	private static final int NUM_TOTAL_DESCONHECIDOS_FATUCOPA = 682;
	private static final int NUM_TOTAL_LANCAMENTOS_FATUCOPA = 157;
	private static final int NUM_TOTAL_LANCAMENTOS_DETAIL_FATUCOPA = 2162;

	private static final int NUM_TOTAL_TITULARES_FATUCOPA_AFTER_USER_RETURN = 466;
	private static final int NUM_TOTAL_DEPENDENTES_FATUCOPA_AFTER_USER_RETURN = 565;
	private static final int NUM_TOTAL_DESCONHECIDOS_FATUCOPA_AFTER_USER_RETURN = 682;
	private static final int NUM_TOTAL_LANCAMENTOS_FATUCOPA_AFTER_USER_RETURN = 157;
	private static final int NUM_TOTAL_LANCAMENTOS_DETAIL_FATUCOPA_AFTER_USER_RETURN = 2162;
	...
	public void testCoparticipacao() throws Exception {
		...
		/*
		 * LOGGER.info("Total titulares ............... [{}]:",
		 * titularUis.size());
		 * LOGGER.info("Total dependentes ............. [{}]:",
		 * dependenteUis.size());
		 * LOGGER.info("Total desconhecidos ........... [{}]:",
		 * desconhecidoUis.size());
		 * LOGGER.info("Total lançamentos ............. [{}]:",
		 * lancamentoUis.size());
		 * LOGGER.info("Total lançamentos details ..... [{}]:",
		 * lancamentoDetailUis.size());
		 * 
		 * Assert.assertEquals(NUM_TOTAL_TITULARES_FATUCOPA, titularUis.size());
		 * Assert.assertEquals(NUM_TOTAL_DEPENDENTES_FATUCOPA,
		 * dependenteUis.size());
		 * Assert.assertEquals(NUM_TOTAL_DESCONHECIDOS_FATUCOPA,
		 * desconhecidoUis.size());
		 * Assert.assertEquals(NUM_TOTAL_LANCAMENTOS_FATUCOPA,
		 * lancamentoUis.size());
		 * Assert.assertEquals(NUM_TOTAL_LANCAMENTOS_DETAIL_FATUCOPA,
		 * lancamentoDetailUis.size());
		 */
	}
