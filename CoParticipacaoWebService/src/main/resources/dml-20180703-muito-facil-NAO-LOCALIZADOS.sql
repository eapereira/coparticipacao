...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default 'dml-20180703-muito-facil-MECSAS.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default 'dml-20180703-muito-facil-NAO-LOCALIZADOS.sql';
	...
	declare VAR_COLUMN_04_VL_PRINCIPAL				bigint( 17 );
	...
	where CD_CONTRATO = 'MECSAS2.muitoFacil.naoLocalizados';
	...
	insert into TB_ARQUIVO_INPUT(
		...
		DT_ALTERED ) values (	
	    VAR_ID_CONTRATO,
		'^NAO\-LOCALIZADO\-MUITO-FACIL\-([0-9]{4})([0-9]{2})\.(xlsx|XLSX)$',
	...
	select max( ID ) into VAR_COLUMN_02_NM_BENEFICIARIO from TB_ARQUIVO_INPUT_COLS_DEF;
	...
	select max( ID ) into VAR_COLUMN_03_NR_CPF from TB_ARQUIVO_INPUT_COLS_DEF;
	...
	select max( ID ) into VAR_COLUMN_04_VL_PRINCIPAL from TB_ARQUIVO_INPUT_COLS_DEF;
