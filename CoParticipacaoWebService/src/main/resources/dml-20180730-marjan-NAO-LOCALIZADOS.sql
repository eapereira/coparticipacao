...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default 'dml-20180730-008-marjan-MECSAS2.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default 'dml-20180730-marjan-NAO-LOCALIZADOS.sql';
	...
	declare VAR_COLUMN_03_NM_BENEFICIARIO			bigint( 17 );
	declare VAR_COLUMN_04_NR_CPF					bigint( 17 );
	...
	where CD_CONTRATO = 'MECSAS2.marjan.naoLocalizados';
	...
	insert into TB_ARQUIVO_INPUT(
		...
		DT_ALTERED ) values (	
	    VAR_ID_CONTRATO,
		'^NAO\-LOCALIZADO\-MARJAN\-([0-9]{4})([0-9]{2})\.(xlsx|XLSX)$',
	...
	select max( ID ) into VAR_COLUMN_02_NR_MATRICULA_EMPRESA from TB_ARQUIVO_INPUT_COLS_DEF;
	...
	select max( ID ) into VAR_COLUMN_03_NM_BENEFICIARIO from TB_ARQUIVO_INPUT_COLS_DEF;
	...
	select max( ID ) into VAR_COLUMN_04_NR_CPF from TB_ARQUIVO_INPUT_COLS_DEF;
	...
	call PROC_LOG_MESSAGE('LINHA - 512');
	insert into TB_BENEFICIARIO_COLS(
		CD_BENEFICIARIO_COLS_DEF,
		ID_ARQUIVO_INPUT_COLS_DEF,
