...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20180924-004-dml-HOC-0444.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20180927-002-dml.sql';
	...
	declare VAR_USE_TYPE_MECSAS_TMP			int( 3 ) default 11;
	declare VAR_USE_TYPE_MECSAS2_TMP		int( 3 ) default 22;
	declare VAR_USE_TYPE_NAO_LOCALIZADO_TMP	int( 3 ) default 33;	
	declare VAR_USE_TYPE_ISENTO_TMP			int( 3 ) default 44;
	declare VAR_USE_TYPE_FATUCOPA_TMP		int( 3 ) default 55;
	
	declare VAR_USE_TYPE_MECSAS_NEW			int( 3 ) default 1;
	declare VAR_USE_TYPE_MECSAS2_NEW		int( 3 ) default 2;
	declare VAR_USE_TYPE_NAO_LOCALIZADO_NEW	int( 3 ) default 3;	
	declare VAR_USE_TYPE_ISENTO_NEW			int( 3 ) default 4;
	declare VAR_USE_TYPE_FATUCOPA_NEW		int( 3 ) default 5;
	...
	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_FATUCOPA_TMP
	where	TP_USE = VAR_USE_TYPE_FATUCOPA;
	
	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_MECSAS_TMP
	where	TP_USE = VAR_USE_TYPE_MECSAS;

	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_MECSAS2_TMP
	where	TP_USE = VAR_USE_TYPE_MECSAS2;
	
	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_ISENTO_TMP
	where	TP_USE = VAR_USE_TYPE_ISENTO;
	
	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_NAO_LOCALIZADO_TMP
	where	TP_USE = VAR_USE_TYPE_NAO_LOCALIZADO;
	
	call PROC_LOG_MESSAGE('LINHA - 288');
	/*********************************************************************************************************************************************/
	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_FATUCOPA_NEW
	where	TP_USE = VAR_USE_TYPE_FATUCOPA_TMP;
	
	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_MECSAS_NEW
	where	TP_USE = VAR_USE_TYPE_MECSAS_TMP;

	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_MECSAS2_NEW
	where	TP_USE = VAR_USE_TYPE_MECSAS2_TMP;
	
	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_ISENTO_NEW
	where	TP_USE = VAR_USE_TYPE_ISENTO_TMP;
	
	update TB_CONTRATO
	set		TP_USE = VAR_USE_TYPE_NAO_LOCALIZADO_NEW
	where	TP_USE = VAR_USE_TYPE_NAO_LOCALIZADO_TMP;
	...
	/*********************************************************************************************************************************************/
	call PROC_LOG_MESSAGE('LINHA - 113');
	update TB_ARQUIVO_INPUT
	set		TP_USE = VAR_USE_TYPE_FATUCOPA_NEW
	where	TP_USE = VAR_USE_TYPE_FATUCOPA_TMP;
	...
	update TB_EMPRESA
	set
			CD_FAILURE_DIR	= '/home/eapereira/desenv/work/coparticipacao/failure/',
			CD_OUTPUT_DIR	= '/home/eapereira/desenv/work/coparticipacao/output/'
