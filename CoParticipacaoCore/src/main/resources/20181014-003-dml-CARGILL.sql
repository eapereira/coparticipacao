...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20181014-002-dml-ABBVIE.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20181014-003-dml-CARGILL.sql';
	...
	set VAR_ID_ARQUIVO_INPUT = FUNC_FIND_ARQUIVO_INPUT( 'CARGILL','NAO-LOCALIZADO');
	...
	select	ID into VAR_COLUMN_06_CD_PLANO
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where 	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_06_CD_PLANO';
	
	call PROC_LOG_MESSAGE('LINHA - 282');
	select	ID into VAR_COLUMN_07_DT_ADMISSAO
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where 	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	...
	set
			CD_TYPE	= VAR_COL_INT
	where	ID		= VAR_COLUMN_06_CD_PLANO;
	
	call PROC_LOG_MESSAGE('LINHA - 258');
	/*********************************************************************************************************************************************/	
    call PROC_LOG_MESSAGE('LINHA - 295');
    update TB_BENEFICIARIO_COLS
    set
    		CD_BENEFICIARIO_COLS_DEF	= VAR_CD_BENEFICIARIO_COLS_DEF_CD_PLANO
	where	ID_ARQUIVO_INPUT_COLS_DEF	= VAR_COLUMN_06_CD_PLANO;
