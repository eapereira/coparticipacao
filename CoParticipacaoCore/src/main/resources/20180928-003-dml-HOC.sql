...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20180928-002-dml-CARGILL.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20180928-003-dml-HOC.sql';
	...
	/*********************************************************************************************************************************************/	
	call PROC_LOG_MESSAGE('LINHA - 185');
	update TB_CONTRATO
	set		
			CD_CONTRATO 	= 'ISENTO-CENTAVOS',
			NM_CONTRATO		= 'ISENTO-CENTAVOS',
			DESCR_CONTRATO	= VAR_NM_CONTRATO_ISENTO
	where	ID				= VAR_ID_CONTRATO;
	
	update	TB_ARQUIVO_INPUT
	set		NM_ARQUIVO_REGEXP	= '^(0444)\\.(ISENTO\\-CENTAVOS)\\.([0-9]{4})([0-9]{2})\\.([0-9]{3})\\.(xlsx|XLSX)$'
	where 	ID_CONTRATO 		= VAR_ID_CONTRATO;
	...
	update TB_CONTRATO
	set		
			CD_CONTRATO = 'ISENTO-UTILIZACAO',
			NM_CONTRATO	= 'ISENTO-UTILIZAÇÃO'
	where	ID			= VAR_ID_CONTRATO;
	
	update	TB_ARQUIVO_INPUT
	set		NM_ARQUIVO_REGEXP	= '^(0444)\\.(ISENTO\\-UTILIZACAO)\\.([0-9]{4})([0-9]{2})\\.([0-9]{3})\\.(xlsx|XLSX)$'
	where 	ID_CONTRATO 		= VAR_ID_CONTRATO;
	...
	insert into TB_REGRA(
		...
		DT_CREATED,
		DT_ALTERED ) values (
		'HOC.ISENTO-CENTAVO - Regra para subtrair o NR_MATRICULA_TITULAR do beneficiário por 44400000000000',
