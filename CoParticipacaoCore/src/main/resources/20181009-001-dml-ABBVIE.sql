...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20181008-003-dml-MARJAN.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20181009-001-dml-ABBVIE.sql';
	...
	set VAR_ID_ARQUIVO_INPUT = FUNC_FIND_ARQUIVO_INPUT( 'ABBVIE','8B1LR');
	...
	select	ID into VAR_ID_COLUMN_14_POSITIVO_NEGATIVO
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where 	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_14_POSITIVO_NEGATIVO';
	
	call PROC_LOG_MESSAGE('LINHA - 280');
	select	ID into VAR_ID_COLUMN_10_CPF_DEPENDENTE
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where 	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_10_CPF_DEPENDENTE';
	
	call PROC_LOG_MESSAGE('LINHA - 286');
	select	ID into VAR_ID_COLUMN_04_NR_MATRICULA_DEPENDENTE
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where 	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_04_NR_MATRICULA_DEPENDENTE';
	...
	call PROC_LOG_MESSAGE('LINHA - 292');
	select	ID into VAR_ID_COLUMN_22_NM_TITULAR
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where 	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_22_NM_TITULAR';
