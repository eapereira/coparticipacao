...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20181013-004-dml-ABBVIE.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20181014-002-dml-ABBVIE.sql';
	...
	call PROC_LOG_MESSAGE('LINHA - 336');	
	/*********************************************************************************************************************************************/
	call PROC_LOG_MESSAGE('LINHA - 280');
	update TB_VIEW_DESTINATION_COLS_DEF
	set
			CD_ORDEM 			= VAR_CD_ORDEM
	where	ID_VIEW_DESTINATION	= VAR_ID_VIEW_DESTINATION
	and		NM_COLUMN			= 'NR_CPF';
	
	set VAR_CD_ORDEM = VAR_CD_ORDEM + 1;
	
	call PROC_LOG_MESSAGE('LINHA - 338');
	update TB_VIEW_DESTINATION_COLS_DEF
	set
			CD_ORDEM 			= VAR_CD_ORDEM
	where	ID_VIEW_DESTINATION	= VAR_ID_VIEW_DESTINATION
	and		NM_COLUMN			= 'DT_ADMISSAO';
	...
	set
			CD_ORDEM 			= VAR_CD_ORDEM
	where	ID_VIEW_DESTINATION	= VAR_ID_VIEW_DESTINATION
	and		NM_COLUMN			= 'NR_MATRICULA_EMPRESA';
	...
	set VAR_ID_ARQUIVO_INPUT = FUNC_FIND_ARQUIVO_INPUT( 'ABBVIE','NAO-LOCALIZADO');
		
	call PROC_LOG_MESSAGE('LINHA - 258');
	select 	ID into VAR_COLUMN_01_NR_MATRICULA
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_01_NR_MATRICULA'; 

	call PROC_LOG_MESSAGE('LINHA - 258');
	select 	ID into VAR_COLUMN_02_NR_CPF
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_02_NR_CPF'; 

	call PROC_LOG_MESSAGE('LINHA - 258');
	select 	ID into VAR_COLUMN_03_NM_DEPENDENTE
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_03_NM_DEPENDENTE'; 

	call PROC_LOG_MESSAGE('LINHA - 258');
	select 	ID into VAR_COLUMN_05_DT_ADMISSAO
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_04_DT_ADMISSAO'; 

	call PROC_LOG_MESSAGE('LINHA - 258');
	select 	ID into VAR_COLUMN_06_NR_UPI
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_05_NR_UPI'; 
	...
	set
			CD_ORDEM			= VAR_CD_ORDEM,
			NM_COLUMN			= 'COLUMN_01_NR_MATRICULA'
	where	ID					= VAR_COLUMN_01_NR_MATRICULA;
	...
	set
			CD_ORDEM			= VAR_CD_ORDEM,
			NM_COLUMN			= 'COLUMN_02_NM_DEPENDENTE'
	where	ID					= VAR_COLUMN_03_NM_DEPENDENTE;
	...
	select 	max( ID ) into VAR_COLUMN_04_NM_TITULAR
	from	TB_ARQUIVO_INPUT_COLS_DEF;
	...
	set
			CD_ORDEM			= VAR_CD_ORDEM,
			NM_COLUMN			= 'COLUMN_04_NR_CPF'
	where	ID					= VAR_COLUMN_02_NR_CPF;
	...
	set
			CD_ORDEM			= VAR_CD_ORDEM,
			NM_COLUMN			= 'COLUMN_05_DT_ADMISSAO'
	where	ID					= VAR_COLUMN_05_DT_ADMISSAO;
	...
	set
			CD_ORDEM			= VAR_CD_ORDEM,
			NM_COLUMN			= 'COLUMN_06_NR_UPI'
	where	ID					= VAR_COLUMN_06_NR_UPI;
	...
	call PROC_LOG_MESSAGE('LINHA - 463');
	/*********************************************************************************************************************************************/
	/*********************************************************************************************************************************************/				
