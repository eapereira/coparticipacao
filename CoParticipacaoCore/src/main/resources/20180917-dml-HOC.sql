...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20180916-004-dml-HOC-ISENTOS-VALOR-CENTAVOS.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20180917-dml-HOC.sql';
	...
	set VL_LENGTH = 40
	where ID_VIEW_DESTINATION in (
		select viewDestination.ID from TB_VIEW_DESTINATION viewDestination
		where viewDestination.NM_VIEW = 'VW_ISENCAO_GESTANTES_HOC'
	)
	and NM_COLUMN in ( 'NM_TITULAR', 'NM_USUARIO' );
