...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20181107-009-dml-TECHNIT-ODONTO-MECSAS.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20181107-010-dml-TECHNIT-ODONTO-091707.sql';
	...
	declare VAR_NR_MATRICULA_BASE											bigint( 17 ) default 917070000000000;
	...
	insert into TB_ARQUIVO_INPUT(
		...
		DT_ALTERED ) values (	
	    VAR_ID_CONTRATO,
		'^(091707)\\.(091707)\\.([0-9]{4})([0-9]{2})\\.([0-9]{3})\\.(xlsx|XLSX)$',
	...
	insert into TB_ARQUIVO_OUTPUT(
		...
		DT_ALTERED ) values (
		VAR_ID_ARQUIVO_INPUT,
		'Techint({CC})-Bradesco (Odonto) - Coparticipação_({YYYY}{MM}).xlsx',
    ...
    where 	NM_VIEW = 'VW_COPARTICIPACAO_TECHNIT_ODONTO';
	...
	select 	ID into VAR_ID_VIEW_DESTINATION
	from 	TB_VIEW_DESTINATION
    where 	NM_VIEW = 'VW_COPARTICIPACAO_RESUMO_TECHNIT_ODONTO';
    ...
    where NM_VIEW = 'VW_DESCONHECIDO_TECHNIT_ODONTO';
	...
	insert into TB_ARQUIVO_OUTPUT_DESCONHECIDO(
		...
		DT_ALTERED ) values (
		VAR_ID_ARQUIVO_INPUT,
		'NAO-LOCALIZADO-TECHINT-ODONTO-{YYYY}{MM}.xlsx',
