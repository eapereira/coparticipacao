...
drop view if exists VW_DESCONHECIDO_LEVEL01_CELPE_ODONTO;
drop view if exists VW_DESCONHECIDO_CELPE_ODONTO;
drop view if exists VW_COPARTICIPACAO_LEVEL01_CELPE_ODONTO;
drop view if exists VW_COPARTICIPACAO_CELPE_ODONTO;
drop view if exists VW_COPARTICIPACAO_RESUMO_CELPE_ODONTO;

/****************************************************************************************************************************************************/
/****************************************************************************************************************************************************/

create view VW_DESCONHECIDO_LEVEL01_CELPE_ODONTO as
select
	desconhecido.CD_MES,
    desconhecido.CD_ANO,
...
where empresa.CD_EMPRESA = 'CELPE_ODONTO'
union all
select
	lancamento.CD_MES,
	lancamento.CD_ANO,
...
create view VW_DESCONHECIDO_CELPE_ODONTO as
select distinct
	desconhecido.CD_MES,
    desconhecido.CD_ANO,
    desconhecido.ID_CONTRATO,
    ...
    desconhecido.DESCR_PROFISSAO,
    desconhecido.NR_MATRICULA_ESPECIAL    
from VW_DESCONHECIDO_LEVEL01_CELPE_ODONTO desconhecido
order by desconhecido.NM_BENEFICIARIO;

create view VW_COPARTICIPACAO_LEVEL01_CELPE_ODONTO as
select
	lancamento.CD_MES,
    lancamento.CD_ANO,
    lancamento.ID_CONTRATO,
...
create view VW_COPARTICIPACAO_CELPE_ODONTO as
select
	FUNC_GET_ROWNUM() ID,
	automind.CD_MES,
    automind.CD_ANO,
	...
	automind.DT_CREATED,
	automind.DT_ALTERED
from	VW_COPARTICIPACAO_LEVEL01_CELPE_ODONTO automind
group by
	automind.CD_MES,
    automind.CD_ANO,
    automind.ID_CONTRATO,
...
create view VW_COPARTICIPACAO_RESUMO_CELPE_ODONTO as
select
	FUNC_GET_ROWNUM() ID,
	automind.CD_MES,
    automind.CD_ANO,
	...
	count( 1 ) / ( select count( 1 ) from VW_COPARTICIPACAO_CELPE_ODONTO ) VL_PROPORCAO,
	automind.VERSION,
	automind.USER_CREATED,
	automind.USER_ALTERED,
	automind.DT_CREATED,
	automind.DT_ALTERED	
from	VW_COPARTICIPACAO_CELPE_ODONTO automind
group by
	automind.CD_MES,
    automind.CD_ANO,
    automind.ID_CONTRATO,
