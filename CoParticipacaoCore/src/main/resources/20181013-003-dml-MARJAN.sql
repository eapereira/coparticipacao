...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20181013-002-dml-MUITO-FACIL.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20181013-003-dml-MARJAN.sql';
	...
	select	ID into VAR_ID_COLUMN_18_NM_TITULAR
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where 	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_18_NM_TITULAR';
	
	call PROC_LOG_MESSAGE('LINHA - 282');
	select	ID into VAR_ID_COLUMN_16_DT_NASCIMENTO
	from 	TB_ARQUIVO_INPUT_COLS_DEF
	where 	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		NM_COLUMN			= 'COLUMN_16_DT_NASCIMENTO';
