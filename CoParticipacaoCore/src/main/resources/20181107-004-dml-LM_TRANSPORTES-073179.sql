...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20181107-003-dml-LM-TRANSPORTES-MECSAS.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20181107-004-dml-LM-TRANSPORTES-073179.sql';
	...
	insert into TB_ARQUIVO_INPUT(
		...
		DT_ALTERED ) values (	
	    VAR_ID_CONTRATO,
		'^(073179)\\.(073179)\\.([0-9]{4})([0-9]{2})\\.([0-9]{3})\\.(xlsx|XLSX)$',
	...
	insert into TB_ARQUIVO_OUTPUT(
		...
		DT_ALTERED ) values (
		VAR_ID_ARQUIVO_INPUT,
		'LMTransportes-Bradesco (Saúde) - Coparticipação_({YYYY}{MM}).xlsx',
    ...
    where 	NM_VIEW = 'VW_COPARTICIPACAO_LM_TRANSPORTES';
	...
	select 	ID into VAR_ID_VIEW_DESTINATION
	from 	TB_VIEW_DESTINATION
    where 	NM_VIEW = 'VW_COPARTICIPACAO_RESUMO_LM_TRANSPORTES';
    ...
    where NM_VIEW = 'VW_DESCONHECIDO_LM_TRANSPORTES';
	...
	insert into TB_ARQUIVO_OUTPUT_DESCONHECIDO(
		...
		DT_ALTERED ) values (
		VAR_ID_ARQUIVO_INPUT,
		'NAO-LOCALIZADO-LM-TRANSPORTES-{YYYY}{MM}.xlsx',
