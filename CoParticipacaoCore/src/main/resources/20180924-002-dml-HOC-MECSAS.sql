...
BEGIN
	declare VAR_NM_SCRIPT_REQUIRED			varchar( 400 ) default '20180923-001-dml-CARGILL-MECSAS.sql';
	declare VAR_NM_SCRIPT					varchar( 400 ) default '20180924-002-dml-HOC-MECSAS.sql';
	...
	set
			CD_TYPE				= VAR_COL_INT
	where	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		ID					= VAR_ID_COLUMN_05_RDP;	

	call PROC_LOG_MESSAGE('LINHA - 381');
	update TB_ARQUIVO_INPUT_COLS_DEF
	set
			CD_TYPE				= VAR_COL_INT
	where	ID_ARQUIVO_INPUT	= VAR_ID_ARQUIVO_INPUT
	and		ID					= VAR_ID_COLUMN_07_COD_LOCAL;	

	call PROC_LOG_MESSAGE('LINHA - 388');
	/*********************************************************************************************************************************************/
	call PROC_LOG_MESSAGE('LINHA - 390');
	delete from TB_BENEFICIARIO_COLS
	where	CD_BENEFICIARIO_COLS_DEF	= VAR_CD_BENEFICIARIO_COLS_DEF_REF_CODE
	and		ID_ARQUIVO_INPUT_COLS_DEF	= VAR_ID_COLUMN_07_COD_LOCAL;
